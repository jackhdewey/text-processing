...
jsPsych.plugins["video-prompt-response"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // VALUATION JUDGEMENT
    video_html += "<div id='jspsych-video-prompt-question' style='display:none;'>" + trial.instruction + '<form><input id="img_payment" name="img_payment" type="number" onkeypress="return event.charCode >= 48 && event.charCode <= 57" min="1" max="100" step="1" style="background-color: #FFFFFF; width: 20%; border-radius: 4px; padding: 10px 10px; margin: 8px 0; border: 1px solid #ccc; font-size: 20px" > minutes</p><span id="displayError" style="color:red"></span><div id="jspsych-video-prompt-response-btngroup"><button id="jspsych-video-button-response-button-submit" type="button" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'">SUBMIT</button></form></div></div>';
          ...
          display_element.querySelector('#jspsych-video-prompt-response-stimulus').pause();
              if(display_box == false)
              {
                ...
                display_element.querySelector('#jspsych-video-button-response-button-submit').addEventListener('click', function(e){
                    if (display_element.querySelector('#img_payment').value > 100){
                        display_element.querySelector('#displayError').innerHTML = 'Input a number less than 100'
