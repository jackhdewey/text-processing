...
//        console.log(block_x, curr_pos)
    ...
    display_element.innerHTML += "<div id='game-instructions' style='display: none'>You will now play a food-hunting game.  You will see a grid of squares, and a yellow disc on one of the squares.<p>The yellow disc represents where YOU are on the grid.  You can move around the grid by using the up, down, left, and right arrow keys.</p><p>To see if the square you are on has food, press the <b>F KEY</b>.  When you move to a square with a piece of food, the square will turn green to indicate you found food.  If the square is empty, you'll be told where the actual food currently is, and then the grid will clear and the food will appear in a different location.  Food appears in clumps -- so if you find food in a square, there will most likely be food close by.  Try to collect as much food as you can!  (You'll see your food score at the top.)<p>Press the ENTER key to begin.</p></div>";
    ...
    display_element.innerHTML += "<div id='fail_text' style='display: none; background-color: green;'>Oops, food was over here</div>";
