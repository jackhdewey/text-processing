...
jsPsych.plugins['forage-grid'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // Get x values
    var sample_x = spacedRandArray(2, 9, 3, 4);
    ...
    // Get y values
    var sample_y = spacedRandArray(2, 9, 3, 4);
...
//        food_pools.push(pool_number);
    ...
    function drawAgent(context){
        ...
        if (isArrayInArray(pools_dict[0][0], pos) || isArrayInArray(pools_dict[0][1], pos)){
            ...
            curr_pool = 1;
        } else if (isArrayInArray(pools_dict[2][0], pos) || isArrayInArray(pools_dict[2][1], pos)){
            curr_pool = 2;
        ...
        if (food_dict[block_id] == 1 && checkForFood == 1){
            checkForFood = 0;
            food_dict[block_id] = 0;
            found_food_latencies.push(performance.now()-start_clock);
            ...
            if (distance >= square_size && distance < square_size*2){
                curr_points = 5;
                subj_score += curr_points;
                display_element.querySelector('#success_text').innerHTML = "So close - you were ONE square away! " + (curr_points).toString() + " points.";
                display_element.querySelector('#success_text').setAttribute("style", "display: block; position: absolute; background-color: #4CAF50; border: none; color: white; padding: 12px 30px; text-decoration: none; top: 10%; left: 30%");
            } else if (distance >= square_size*2 && distance < square_size*3){
                curr_points = 3;
                subj_score += curr_points;
                display_element.querySelector('#success_text').innerHTML = "You were TWO squares away! " + (curr_points).toString() + " points.";
                display_element.querySelector('#success_text').setAttribute("style", "display: block; position: absolute; background-color: #4CAF50; border: none; color: white; padding: 12px 30px; text-decoration: none; top: 10%; left: 30%");
            } else if (distance >= square_size*3 && distance < square_size*4){
                curr_points = 1;
                subj_score += curr_points;
                display_element.querySelector('#success_text').innerHTML = "You were THREE squares away! " + (curr_points).toString() + " point.";
                display_element.querySelector('#success_text').setAttribute("style", "display: block; position: absolute; background-color: #4CAF50; border: none; color: white; padding: 12px 30px; text-decoration: none; top: 10%; left: 30%");
            } else if (distance >= square_size*4 && distance < square_size*6){
                curr_points = 1;
                subj_score += curr_points;
                display_element.querySelector('#success_text').innerHTML = "Close, but not quite." + (curr_points).toString() + " point.";
                display_element.querySelector('#success_text').setAttribute("style", "display: block; position: absolute; background-color: #4CAF50; border: none; color: white; padding: 12px 30px; text-decoration: none; top: 10%; left: 30%");
            } else {
                curr_points = 0;
                subj_score += 0;
                display_element.querySelector('#fail_text').innerHTML = "Oops, you were too far away!";
...
//                    context.closePath();
    ...
    display_element.innerHTML += "<div id='game-instructions' style='display: none'>You will now play a food-hunting game.  You will see a grid of squares, and your task is to find the square with a piece of food (there'll only be one piece at a time).<p>You can use your mouse cursor to click a square to find food. When you move to a square with a piece of food, the square will turn green. If the square has no food, the square instead will turn red, and you will receive feedback for how close you were to the food item.<p>The actual location of the food item will reset every time you click a square. <b>But the locations of the food are clumped together over time -- so the next piece of food will often be close to the previous location of the food.</b>Sometimes, even though you click around the area, you might get a note that you're far away from the piece of food again.  This means that the food clump has shifted to a different location in the board, so you have to find that food clump again.</p><p>The number of points you get will depend on how close you are to the food. Try to earn as many points as you can! <b>Top scorers will earn bonus pay. (In particular, after 2 weeks, the 5 highest scorers will each receive 50% bonus pay.)</b><p>You'll see your food score at the top, and a progress bar at the bottom so you can see how much time there is left to this section.  Good luck!</p><p id='start-key' style='color: grey'><b>Press the ENTER key to begin.</p></b></div>";
    ...
    setTimeout(function(){
        ...
        setTimeout(function(){
            display_element.querySelector('#game-instructions').setAttribute("style", "display: block; border-radius: 4px; padding: 10px 10px 20px 20px; background-color: white; width: 800px; height: 525px; position: absolute; top: 22%; left: "+block_center+"px");
