...
jsPsych.plugins['forage-grid'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // Get x values
    var sample_x = spacedRandArray(2, 9, 3, num_pools);
...
//    var rand_x4 = sample_x[3];
    ...
    // Get y values
    var sample_y = spacedRandArray(2, 9, 3, num_pools);
...
//    var center4 = [rand_x4, rand_y4];
    var centers = [center1, center2, center3];
    console.log("CENTERS: ", center1, center2, center3);
...
//        }
        ...
        if (food_dict[block_id] == 1 && checkForFood == 1){
            ...
            else if (distance >= square_size*3 && distance < square_size*5){
                curr_points = 1;
                subj_score += curr_points;
...
//            "a_pool4": pools_dict[3],
    ...
    display_element.innerHTML += "<div id='game-instructions' style='display: none'>You will now play a food-hunting game.  You will see a grid of squares, and your task is to find the square with a piece of food (there'll only be one piece at a time).<p>You can use your mouse cursor to click a square to find food. When you move to a square with a piece of food, the square will turn green. If the square has no food, the square instead will turn red, and you will receive feedback for how close you were to the food item.<p>The actual location of the food item will change every time you click a square. <b>But the locations of the food are clumped together -- so the next piece of food will often be close to the location of the previous food.</b>  Sometimes, even though you think you've found an area of food, you might get a note that you're far away from the piece of food again.  This means that the food clump has shifted to a different location in the board, so you have to find the new food clump again.</p><p>The number of points you get will depend on how close you are to the food. Try to earn as many points as you can! <b>Top scorers will earn bonus pay. (In particular, after 2 weeks, the 5 highest scorers will each receive 50% bonus pay.)</b><p>You'll see your food score at the top, and a progress bar at the bottom so you can see how much time there is left to this section.  Good luck!</p><p id='start-key' style='color: grey'><b>Press the ENTER key to begin.</p></b></div>";
    display_element.innerHTML += "<div id='foraging_display' style='display: none'>Your score: <span id='score_text'>"+subj_score+"</span><br><span style='font-size: 12px'>(TOP SCORE: 784)</span></div>" + "<div id='canvas_display' style='display: none;'>"+"<canvas id='myCanvas' width='"+trial.canvas_size[0]+"' height='"+trial.canvas_size[1]+"'></canvas>"+"</div>"+"</div>";
