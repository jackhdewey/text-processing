...
jsPsych.plugins['forage-grid'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var circle_size = 4;
    var block_center = screen.width/2 - 425;
    ...
    // Get x values
    var sample_x = spacedRandArray(1, 8, 2, num_pools);
    var rand_x1 = get_sample(range(1, 3), 1)[0];//sample_x[0];
    var rand_x2 = get_sample(range(6, 8), 1)[0];//sample_x[1];
    var rand_x3 = get_sample(range(1, 3), 1)[0];//sample_x[2];
    var rand_x4 = get_sample(range(6, 8), 1)[0];//sample_x[3];

    // Get y values
    var sample_y = spacedRandArray(1, 8, 2, num_pools);
    var rand_y1 = get_sample(range(1, 3), 1)[0];//sample_y[0];
    var rand_y2 = get_sample(range(1, 3), 1)[0];//sample_y[1];
    var rand_y3 = get_sample(range(6, 8), 1)[0];//sample_y[2];
    var rand_y4 = get_sample(range(6, 8), 1)[0];//sample_y[3];

    // Four centers
    var center1 = [rand_x1, rand_y1];
    var center2 = [rand_x2, rand_y2];
    var center3 = [rand_x3, rand_y3];
    ...
    var local_pool_surround = [];
    for (i=0; i<num_pools; i++){
        var loc_center = centers[i];
        var pool_inner = [[loc_center[0], loc_center[1]]];
        var pool_outer = [[loc_center[0]-1, loc_center[1]], [loc_center[0]+1, loc_center[1]],
        [loc_center[0]-1, loc_center[1]+1], [loc_center[0], loc_center[1]+1], [loc_center[0]+1, loc_center[1]+1],
        [loc_center[0]-1, loc_center[1]-1], [loc_center[0], loc_center[1]-1], [loc_center[0]+1, loc_center[1]-1]];
        var pool_surround = [[loc_center[0]-2, loc_center[1]-2], [loc_center[0]-1, loc_center[1]-2], [loc_center[0], loc_center[1]-2], [loc_center[0]+1, loc_center[1]-2], [loc_center[0]+2, loc_center[1]-2], [loc_center[0]-2, loc_center[1]-1], [loc_center[0]+2, loc_center[1]-1],[loc_center[0]-2, loc_center[1]], [loc_center[0]+2, loc_center[1]], [loc_center[0]-2, loc_center[1]+2], [loc_center[0]-1, loc_center[1]+2], [loc_center[0], loc_center[1]+2], [loc_center[0]+1, loc_center[1]+2], [loc_center[0]+2, loc_center[1]+2], [loc_center[0]-2, loc_center[1]+1], [loc_center[0]+2, loc_center[1]+1]];
        pools_dict[i] = [pool_inner, pool_outer, pool_surround];
    }
    ...
    local_pool_surround = pools_dict[pool_number][2];
...
//        food_pools.push(pool_number);
    ...
    function drawAgent(context){
        ...
        food_times.push(performance.now() - start_clock);
        food_pools.push(pool_number);
        food_pool_ids.push(total_pools);
        ...
        if (food_dict[block_id] == 1 && checkForFood == 1){
            ...
            if (isArrayInArray(local_pool_inner, pos)) {
                ...
                display_element.querySelector('#success_text').innerHTML = "So close - it was right next to you! " + (curr_points).toString() + " points.";
                display_element.querySelector('#success_text').setAttribute("style", "display: block; position: absolute; background-color: #4CAF50; border: none; color: white; padding: 12px 30px; text-decoration: none; top: 10%; left: 30%");
            } else if (isArrayInArray(local_pool_surround, pos)){
                curr_points = 1;
                subj_score += curr_points;
                display_element.querySelector('#fail_text').innerHTML = "Not quite! 1 point.";
...
//            context.fill();
    ...
    function startForaging(which_part){
        ...
        // Obtained food
        found_food_latencies = [];
        found_food_pools = [];
    
        // Matching pool
        start_pool_clock = performance.now();
        found_pool = 0;
        found_pool_latencies = [];
        ...
        jsPsych.pluginAPI.setTimeout(function() {
            ...
            if (which_part==1){
                trial_data_a = {
                    ...
                    "food_pool_ids": food_pool_ids,
                };
                console.log(trial_data_a)
                display_element.querySelector('#foraging_display').setAttribute("style", "display: none;");
                display_element.querySelector('#myProgress').setAttribute("style", "display: none;");
                display_element.querySelector('#myBar').setAttribute("style", "display: none;");
...
//                    context.closePath();
    ...
    function endTrial() {
        ...
        var trial_data = {
            ...
            "a_food_pool_ids": trial_data_a["food_pool_ids"],
            ...
            "b_pool_pattern": trial_data_b["pool"],
            "b_food_score": trial_data_b["food_score"],
            "b_total_score": trial_data_b["food_score"],
            "b_move_list": trial_data_b["move_list"],
            "b_move_times": trial_data_b["move_times"],
            "b_move_pools": trial_data_b["move_pools"],
            "b_move_points": trial_data_b["move_points"],
            "b_move_found": trial_data_b["move_found_food"],
            "b_food_list": trial_data_b["food_list"],
            "b_food_times": trial_data_b["food_times"],
            "b_food_pools": trial_data_b["food_pools"],
            "b_food_pool_ids": trial_data_b["food_pool_ids"],
            "b_found_pools": trial_data_b["found_pool_latencies"],
            "b_pool1": pools_dict[0],
            "b_pool2": pools_dict[1],
            "b_pool3": pools_dict[2],
            "b_pool4": pools_dict[3],
        };
