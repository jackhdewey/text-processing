...
jsPsych.plugins["cpt"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var new_html = "<div style='position: absolute; top: 10%; left: 33%'>Press the <b>J key</b> if female.  Do <b>NOT</b> press anything if male.</div><div class='img-wrapper'>";
...
//            response_dict[i] = array;
        ...
        console.log({
          idx: window._pressCtr++,
          image_id: old_imgidx-1,
          condition: conditions[old_imgidx-1],
          evt: 'push',
          k: evt.keyCode
        });
    };
    ...
    {
      document.addEventListener('keypress', window.update_response, true);
      window._press = [];
...
//        "image_list": image_list,
        "total_presses": window._pressCtr,
      ...
      for (i=0; i<trial.num_faces; i++){
        ...
        if (response_dict[i].length==0){
            console.log("Empty array", response_dict[i]);
            if (i > 0){
                if (response_dict[i-1].length > 1){
                    trial_data[selected_col_name] = response_dict[i-1][response_dict[i-1].length - 1];
                    console.log("Previous press:", response_dict[i-1][response_dict[i-1].length - 1]);
                } else {
                    trial_data[selected_col_name] = 0;
                }
            } else { trial_data[selected_col_name] = 0;}
        } else {
            console.log("Not an empty array", response_dict[i]);
