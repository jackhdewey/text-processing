...
jsPsych.plugins["video-prompt-response"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // VALUATION JUDGEMENT
    video_html += "<div id='jspsych-video-prompt-wrapper' style='display:none;'>" + trial.instruction + '<form><input id="img_payment" name="img_payment" type="number" onkeypress="return event.charCode >= 48 && event.charCode <= 57" min="1" max="999" step="1" style="background-color: #FFFFFF; width: 20%; border-radius: 4px; padding: 10px 10px; margin: 8px 0; border: 1px solid #ccc; font-size: 20px" > days</p><span id="displayError" style="color:red"></span><div id="jspsych-video-prompt-response-btngroup"><button id="jspsych-video-button-response-button-submit" type="button" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'">SUBMIT</button></form></div></div>';
    ...
    if(trial.autoplay !== true){
      setTimeout(function(){
          display_element.querySelector('#start-prompt').setAttribute("style", "color: black;");
        ...
        if(currenttime >= trial.stop)
        {
          ...
          document.addEventListener("keypress", disable_enter);
              if(display_box == false)
              {
                ...
                display_element.querySelector("#jspsych-video-prompt-wrapper").setAttribute("style", "display: block; width: "+trial.box_dims[0]+"; opacity:0.9; border-radius: 4px; padding: 10px 10px 20px 20px; background-color: white; position: absolute; top: 10%; left: "+left_align+"px");
                display_element.querySelector('#jspsych-video-button-response-button-submit').addEventListener('click', function(e){
                    if (display_element.querySelector('#img_payment').value > 999){
                        display_element.querySelector('#displayError').innerHTML = 'Input a number less than 999'
                    } else if (display_element.querySelector('#img_payment').value <= 0){
                        display_element.querySelector('#displayError').innerHTML = 'Input a number larger than 0'
                    } else { after_response();};
                });
                display_box = true;
              }
